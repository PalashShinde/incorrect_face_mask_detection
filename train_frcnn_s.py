...
from keras_frcnn import config, data_generators_old
...
train_imgs = [s for s in all_imgs if s['imageset'] == 'trainval' and 'test' ]
...
data_gen_train = data_generators_old.get_anchor_gt(
    train_imgs, classes_count, C, nn.get_img_output_length, K.image_dim_ordering(), mode='train')
# data_gen_val = data_generators_old.get_anchor_gt(
...
for epoch_num in range(num_epochs):
    ...
    while True:
        try:
            ...
            print('data_gen_train_X',X.shape)
            print('data_gen_train_Y',Y.shape)
            ...
            if iter_num == epoch_length:
                ...
                if curr_loss < best_loss:
                    ...
                    best_loss = curr_loss
                    model_all.save_weights(
                        dir1+"bacteria-total-101-march28-" + str(epoch_num) + '_' + str(curr_loss) + ".hdf5")
                    EpochsSinceCheckpoint = 0
                else:
